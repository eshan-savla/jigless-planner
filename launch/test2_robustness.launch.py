...
def generate_launch_description():
    ...
    declare_freq = DeclareLaunchArgument(
        'freq',
        default_value="0.05",
        description='Frequency of plan change'
    )

    declare_duration = DeclareLaunchArgument(
        'duration',
        default_value="120",
        description='Duration of plan change test'
    )
    ...
    test_launch_service = Node(
        package='jigless-planner',
        executable='test2_client_node.py',
        name='test2_robustness_node',
        namespace='',
        output='screen',
        parameters=[
            {
 ...
 # Create the launch description and populate
    ...
    ld.add_action(declare_duration)
